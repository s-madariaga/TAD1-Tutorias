...
<img src="img/R.png" width="200"/> <img src="img/rstudio.png" width="350"/>
:::
...
<center><img src="img/dataframes.webp" width="550"/></center>

-   Los objetos tipo `data.frame` (por su traducción: "*marco de datos"*) son los objetos reconocidos como bases de datos.

-   Cada columna es una **variable**. Las variables, a su vez, son vectores y heredan todas las caracteristicas de los vectores. <!-- "vectores con nombres" podemos decir -->

-   Se pueden configurar a mano:
...
-   Pero comunmente se importan como archivo externo.
    -   Aquí sí es muy importante el directorio.

```{r, eval = FALSE}
setwd("dirección de la carpeta con el archivo")
...
-   Es importante echar un vistazo al interior.

La estrucura de un marco de datos es similar a una matriz, con la diferencia que es un objeto de datos **heterogéneo**. Además, las columnas de un marco de datos adquieren una interpretación distinta: las filas son observaciones (unidades de observación) y las columnas son variables.
...
Las funciones `head()` nos permite ver las primeras 5 observaciones de la base de datos. La función `tail()` nos permite ver las últimas 5 observaciones de la base de datos.
...
##### 5.3.1. Comprobar el tipo de dato de las variables

La funciòn `str()` nos permite ver las variables y la clase de las mismas.

```{r}
str(data)
```

##### 5.3.2. Manipulación de las variables

Los marcos de datos en R tienen sus propios métodos para consultar la posición de los valores, en tanto son variables y observaciones.
...
Por ejemplo, podemos usar la función `factor()` para asignar la clase de variable categórica a una variable.
...
Usamos condiciones lógicas para constrir filtros de las observaciones de una base de datos.

-   Lo que construye el filtro es el vector de valores booleanos generado.

-   Podemos articular condiciones con los operadores `|` y `&`.

```{r}
data[data$edad > 18, ]
data[data$edad > 30 & data$sexo == "hombre", ]
```

------------------------------------------------------------------------

#### 5.4. Otras estructuras de datos

Existen otras estrcutras como **listas** y **arrays**, que son menos relevantes en R.
...
Todo este tiempo hemos estado usando **funciones**. Las funciones en R son métodos que podemos aplicar a los objetos. Son algunos ejemplos:

-   `mean()`: promedio de una variable. Se aplica a vectores de clase numérica, que suponen (sustantivas) mediciones continuas.

-   `sd():` desviación estándar de una variable. Se aplica a vectores de clase numérica, que suponen (sustantivas) mediciones continuas.

-   `table()`: tabulación de una variable. Se aplica a vectores con valores recurrentes, como variables categóricas y discretas. Los datos pueden tener cualquier clase.
...
Podemos acceder a la **documentación** de estas funciones, disponibles en [r-proyect.org](https://cran.r-project.org/).
