...
c("Manzana", "Naranja", "Plátano", "Palta") %>% str_replace_all("a", "")
...
# n. Manejo de directorios y documentos -----------------------------------
...
for(database in my_files){
  databases[[database]] <- read_csv(paste0("Input/",database))
}
...
##~~ Proceso individual ~~~~~~~~~~~~~
netflix <- netflix %>%
  mutate(mes = month(mdy(date_added), label = TRUE),
...
procesamiento <- function(dir){
  ...
  for(database in 1:length(my_files)){
...
procesamiento(dir = "Input/")

# Función corregida:
file.edit("Input/automatizacion_correcta.r")
