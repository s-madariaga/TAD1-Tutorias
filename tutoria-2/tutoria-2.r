...
## Importamos la base de datos: pero esto no es correcto.
...
netflix %>% 
  sapply(mi_funcion) ## Para usar esta variable debo aplicar una función, pero ¿qué función?

netflix %>% 
  summarise_all(mi_funcion)
...
netflix %>% 
  mutate(id = 1:8807) # ¿Hay una mejor manera de generar esta variable?
...
netflix %>% 
  ...
  arrange(release_year) ## ¿Cómo puedo hacer para que salgna las más nuevas?
...
## Ejercicio 1:
## Cuántas películas han sido añadidas por netflix los años 2020, 2021 y 2022 y clasifique entre películas y series.
